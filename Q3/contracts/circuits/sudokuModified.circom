...
template sudoku() {
    ...
    component rangeProofPuzzleValidations[9][9];
    component rangeProofSolutionValidations[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeProofPuzzleValidations[i][j] = RangeProof(32);

            rangeProofPuzzleValidations[i][j].in <== puzzle[i][j];
            rangeProofPuzzleValidations[i][j].range[0] <== 0;
            rangeProofPuzzleValidations[i][j].range[1] <== 9;

            rangeProofSolutionValidations[i][j] = RangeProof(32);

            rangeProofSolutionValidations[i][j].in <== solution[i][j];
            rangeProofSolutionValidations[i][j].range[0] <== 0;
            rangeProofSolutionValidations[i][j].range[1] <== 9;

            assert(rangeProofSolutionValidations[i][j].out * rangeProofPuzzleValidations[i][j].out);
